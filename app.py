...
from Modules.folder_browser import select_recordings_folder
...
# Get the selected folder from the folder browser
folder_target = select_recordings_folder()
...
icon_photos = [] # Stores the lil VLC Icons
...
window.title("Shadowplayer: " + folder_target)
...
# Set the taskbar icon using the same icon file
ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID(icon_file)

# Set frame color and background color
window.configure(bg="#CCCCCC")
window.attributes("-alpha", 1)  # Adjust window opacity if desired
...
def update_thumbnail_border(index):
    ...
    if checkbox_vars[index].get():
        thumbnail_label.config(borderwidth=4, relief="sunken", highlightbackground="green", highlightcolor="green", highlightthickness=6)
    else:
        thumbnail_label.config(borderwidth=4, relief="ridge", highlightthickness=0)
...
for i, video in enumerate(video_data):
    ...
    # Load the icon image
    icon_image = Image.open("vlc48x48.ico")
    icon_image = icon_image.resize((24, 24), Image.LANCZOS)  # Resize the image if necessary

    # Create the PhotoImage from the icon image and store it
    icon_photo = ImageTk.PhotoImage(icon_image)
    icon_photos.append(icon_photo)

    # Add clickable icon on top of the thumbnail
    icon_button = tk.Button(window, image=icon_photo, command=lambda path=video["filepath"]: play_video(path))
    icon_button.grid(row=i // 4, column=i % 4)
...
# Video name entry
name_label = tk.Label(control_frame, text="Output File(s) Prefix:")
